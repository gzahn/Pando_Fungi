...
p1 <- plotQualityProfile(fns[c(1,66)]) + ggtitle("Example forward reads")

# display and save the plots
p1
ggsave("./Output/Figs/unfiltered_quality_plots_2024.png",dpi=500,height = 6,width = 6)
...
out <- filterAndTrim(fns, filts_f, # input and output file names as denoted above
                     ...
                     maxEE=5, # refers to the maximum expected errors allowed
...
orig_filtsf <- basename(filts_f) %>% str_remove("_FWD_filt.fastq.gz")
remaining_filtsf <- row.names(out)[remaining_samples] %>% str_remove("_cutadapt_fwd.fastq.gz")

# In case some samples may have had zero reads pass QC, reassign filts
filts_f <- filts_f[orig_filtsf %in% remaining_filtsf]
...
# REMOVE CHIMERAS ####
seqtab.nochim <- removeBimeraDenovo(seqtab_2024, method="consensus", multithread=TRUE, verbose=TRUE)
saveRDS(seqtab.nochim,"./Output/seqtab.nochim_2024.RDS")
dim(seqtab.nochim)
sum(seqtab.nochim)/sum(seqtab_2024)
...
saveRDS(seqtab.nochim,"./Output/seqtab.nochim.clean_2024.RDS")
dim(seqtab.nochim)
...
write.csv(contams, file = "./Output/likely_contaminants_2024.csv", row.names = TRUE)
...
# reload point
saveRDS(seqtab.nochim,"./Output/seqtab_for_taxonomy_2024.RDS")
seqtab.nochim <- readRDS("./Output/seqtab_for_taxonomy_2024.RDS")
meta <- meta[meta$sample %in% row.names(seqtab.nochim),]
...
tax <- readRDS("./Output/sequence_taxonomy_2024.RDS")
dim(seqtab.nochim)
dim(meta)
dim(tax)
...
saveRDS(ps,"./Output/phyloseq_object_not-cleaned_2024.RDS")
...
saveRDS(ps,"./Output/clean_phyloseq_object_2024.RDS")

# make "melted" version for export as well
melt <- psmelt(ps)
saveRDS(melt,"./Output/clean_phyloseq_melted_df_2024.RDS")

ps1 <- readRDS("./Output/clean_phyloseq_object.RDS")
ps2 <- ps
full <- merge_phyloseq(ps1,ps2)
saveRDS(full,"./Output/clean_phyloseq_object.RDS")

full@sam_data %>% 
  as('data.frame') %>% 
  group_by(seq_date,sample_type) %>% 
  summarize(N=n())
